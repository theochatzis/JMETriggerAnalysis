...
for recoKey in "${recoKeys[@]}"; do
  for RMSEtaSF_factor in  "${RMSEtaSF_factors[@]}"; do
    for MedEtaSF_factor in "${MedEtaSF_factors[@]}"; do
      ...
      python ${CMSSW_BASE}/src/JMETriggerAnalysis/NTuplizers/test/jmeTriggerNTuple_cfg_puppi_less.py dumpPython=.tmp_cfg.py numThreads=1 reco=${recoKey} keepPFPuppi=False puppiParamsHE1=MinNeutralPt:1.0,MinNeutralPtSlope:7.0 \
             puppiParamsHB=MinNeutralPt:1.0,MinNeutralPtSlope:2.0 \
