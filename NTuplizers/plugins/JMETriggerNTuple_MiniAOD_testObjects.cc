...
class JMETriggerNTuple_MiniAOD_testObjects : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit JMETriggerNTuple_MiniAOD_testObjects(const edm::ParameterSet&);
...
protected:
  ...
  float leadingJetPt_ = 0.;
  float leadingJetEta_ = 0.;
  float leadingJetPhi_ = 0.;
  float leadingJetMass_ = 0.;
  double met_ = 0.;
  double metPhi_ = 0.;
  double metNoMu_ = 0.;
  double ht_ = 0.;
  ...
  std::vector<float> offlineAK4PFPuppiJetsCorrected_pt;
  std::vector<float> offlineAK4PFPuppiJetsCorrected_eta;
  std::vector<float> offlineAK4PFPuppiJetsCorrected_phi;
  std::vector<float> offlineAK4PFPuppiJetsCorrected_mass;
...
JMETriggerNTuple_MiniAOD_testObjects::JMETriggerNTuple_MiniAOD_testObjects(const edm::ParameterSet& iConfig)
...
void JMETriggerNTuple_MiniAOD_testObjects::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  run_ = iEvent.id().run();
  ...
  if (not triggerResults_handle.isValid()) {
    edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::analyze")
    ...
    LogDebug("JMETriggerNTuple_MiniAOD_testObjects::analyze") << "output collections will be saved to TTree";
  ...
  if(createTriggerQuantities_){
    ...
    for (const pat::TriggerObjectStandAlone& triggerObject : *triggerObjects) {
      ...
      // loop over trigger names and find if the object matches any of them
      for (auto const& iTriggerPathName : triggerNamesForObjects) {
        hasTriggerName |= triggerObjectNonConst.hasPathName(iTriggerPathName);
      }
...
template <typename... Args>
void JMETriggerNTuple_MiniAOD_testObjects::addBranch(const std::string& branch_name, Args... args) {
  if (ttree_) {
    if (std::find(outputBranchesToBeDropped_.begin(), outputBranchesToBeDropped_.end(), branch_name) ==
      ...
      if (ttree_->GetBranch(branch_name.c_str())) {
        throw cms::Exception("JMETriggerNTuple_MiniAOD_testObjects::addBranch")
      ...
      edm::LogInfo("JMETriggerNTuple_MiniAOD_testObjects::addBranch")
          << "output branch \"" << branch_name
          << "\" will not be created (string appears in data member \"outputBranchesToBeDropped\")";
    }
  } else {
    edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::addBranch")
        << "pointer to TTree is null, output branch \"" << branch_name << "\" will not be created";
  }
}

bool JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_OR(const edm::TriggerResults& triggerResults,
                                               const edm::Event& iEvent,
                                               const std::vector<std::string>& paths) {
  if (paths.empty()) {
    edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_OR")
  ...
  for (unsigned int idx = 0; idx < triggerResults.size(); ++idx) {
    if (triggerResults.at(idx).accept() == true) {
      ...
      if (std::find(paths.begin(), paths.end(), triggerName) != paths.end()) {
        LogDebug("JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_OR") << "event accepted by path \"" << triggerName << "\"";
        return true;
      } else {
        ...
        if (std::find(paths.begin(), paths.end(), triggerName_unv) != paths.end()) {
          LogDebug("JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_OR")
...
bool JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_AND(const edm::TriggerResults& triggerResults,
                                                const edm::Event& iEvent,
                                                const std::vector<std::string>& paths) {
  if (paths.empty()) {
    edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_AND")
        << "input error: empty list of paths for event selection, will return True";
    return true;
  }
  ...
  for (unsigned int idx = 0; idx < triggerResults.size(); ++idx) {
    if (triggerResults.at(idx).accept() == false) {
      ...
      if (std::find(paths.begin(), paths.end(), triggerName) != paths.end()) {
        LogDebug("JMETriggerNTuple_MiniAOD_testObjects::passesTriggerResults_AND")
...
JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::FillCollectionConditionsMap() { this->clear(); }

int JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::init(const edm::ParameterSet& pset) {
  this->clear();
...
void JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::clear() { condMap_.clear(); }

bool JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::has(const std::string& name) const {
  return (condMap_.find(name) != condMap_.end());
}

const JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::condition& JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::at(
...
bool JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::accept(const std::string& name) const {
  if (not this->has(name)) {
    throw cms::Exception("LogicError") << "internal map has no entry associated to key \"" << name << "\"";
  }
...
int JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap::update(const edm::TriggerResults& triggerResults,
                                                          const edm::Event& iEvent) {
...
template <typename VAL_TYPE>
int JMETriggerNTuple_MiniAOD_testObjects::initValueContainers(std::vector<ValueContainer<VAL_TYPE>>& v_valContainers,
...
template <typename VAL_TYPE>
void JMETriggerNTuple_MiniAOD_testObjects::fillValueContainers(
    std::vector<ValueContainer<VAL_TYPE>>& v_valContainers,
    const JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap& fillCollectionConditionMap,
    const edm::Event& iEvent) {
  for (auto& valueContainer_i : v_valContainers) {
    ...
    if (not i_handle.isValid()) {
      edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::fillValueContainers")
...
template <typename COLL_CONTAINER_TYPE, typename OBJ_TYPE>
int JMETriggerNTuple_MiniAOD_testObjects::initCollectionContainer(
    const edm::ParameterSet& iConfig,
  ...
  if (iConfig.exists(collPSetName)) {
    ...
    for (auto const& label : inputTagLabels_collections) {
      ...
      LogDebug("JMETriggerNTuple_MiniAOD_testObjects::initCollectionContainer")
...
template <typename COLL_CONTAINER_TYPE, typename OBJ_TYPE>
void JMETriggerNTuple_MiniAOD_testObjects::fillCollectionContainer(edm::Event const& iEvent,
                                               std::vector<COLL_CONTAINER_TYPE>& v_collectionContainer,
                                               JMETriggerNTuple_MiniAOD_testObjects::FillCollectionConditionsMap const& fillConditionMap) {
  for (auto& collContainer_i : v_collectionContainer) {
    ...
    if (i_handle.isValid()) {
      ...
      edm::LogWarning("JMETriggerNTuple_MiniAOD_testObjects::fillCollectionContainer")
...
void JMETriggerNTuple_MiniAOD_testObjects::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add("JMETriggerNTuple_MiniAOD_testObjects", desc);
}

DEFINE_FWK_MODULE(JMETriggerNTuple_MiniAOD_testObjects);
